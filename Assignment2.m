classdef Assignment2
    ...
    methods
        ...
        function self = commandGrippers(self, command)
            ...
            switch(command)
                case 'update'
                    ...
                    self.gripperLeft1.model.animate(self.gripperLeft1.model.getpos());
                    self.gripperRight1.model.animate(self.gripperRight1.model.getpos());
        ...
        function self = plotRadAndVol(self)
            ...
            disp('UR3e linear rail is  0.8m.')
            disp(' ')
            disp('The robot cannot reach below the table, therefore the volume of its')
            disp('reach is determined by two half-ellipsoids on each end of the rail, ');
            disp('and a half-cylinder along the length of the cylinder, all with a major ')
            disp('radius of 0.9410m and minor radius of 0.7410m.')
            disp(' ')
            disp('All together, this represents a working volume of:')
            disp('[(2π/3)*(0.9410 * 0.7410^2) + (π/2)*(0.9410 * 0.7410 * 0.8)]')
            disp(['    = ',num2str((2*pi/3)*(0.9410 * 0.7410^2) + (pi/2)*(0.9410 * 0.7410 * 0.8)),' cubic metres']);
